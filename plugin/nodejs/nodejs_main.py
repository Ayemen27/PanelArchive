...
#             public.ExecShell(exec_cmd)
        if not os.path.exists(mod_pack_file):
            return self.return_data(False,res,error_msg='installation failed!')
        ...
        try:
            ...
            if get:
                ...
                if not os.path.exists(src_node_bin):
                    return self.return_data(False,'',error_msg='The specified version does not exist!')
            ...
            return self.return_data(False, error_msg='<a style="color:red;">Error: Setting failed, /usr/bin is not writable!</a><br>1. If [Pagoda System Reinforcement] is installed, please close it first<br>2. If If Cloud Lock is installed, please turn off the [System Hardening] function<br> 3. If a security dog ​​is installed, please turn off the [System Protection] function<br> 4. If you use other security software, please uninstall it first<br>')
            # return self.return_data(False, error_msg='<a style="color:red;">错误：设置失败，/usr/bin不可写!</a><br>1、如果安装了[宝塔系统加固]，请先关闭<br>2、如果安装了云锁，请关闭[系统加固]功能<br>3、如果安装了安全狗，请关闭[系统防护]功能<br>4、如果使用了其它安全软件，请先卸载<br>')