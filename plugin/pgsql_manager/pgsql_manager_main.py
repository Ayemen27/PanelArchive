...
class pgsql_manager_main:
    ...
    def Install_uninstall(self, args):  # 安装卸载pgsql
        ...
        if int(args.get_pgsql_version) != 0 and int(args.get_pgsql_install_info) != 0:
            ...
            pgsql_install_info = [
                {"pgsql_version": "postgresql 16.1", "down_url": bt_down_url + "/src/postgresql-16.1.tar.gz"},
                {"pgsql_version": "postgresql 15.5", "down_url": bt_down_url + "/src/postgresql-15.5.tar.gz"},
                {"pgsql_version": "postgresql 14.10", "down_url": bt_down_url + "/src/postgresql-14.10.tar.gz"},
                {"pgsql_version": "postgresql 13.13", "down_url": bt_down_url + "/src/postgresql-13.13.tar.gz"},
                {"pgsql_version": "postgresql 12.17", "down_url": bt_down_url + "/src/postgresql-12.17.tar.gz"},
                {"pgsql_version": "postgresql 11.22", "down_url": bt_down_url + "/src/postgresql-11.22.tar.gz"},
            ...
            match = re.search(r'\(PostgreSQL\) (\d+)', pgsql_version)
            if match:
                pg_main_version = match.group(1)
                for pg_info in pgsql_install_info:
                    if re.search(r'\b{}\b'.format(pg_main_version), pg_info["pgsql_version"]):
                        pgsql_version = f"psql (PostgreSQL) {pg_info['pgsql_version'].split()[-1]}\n"
                        break
            ...
            public.ExecShell('''chmod 755 /usr/local/usranalyse/lib/libusranalyse.so''')
