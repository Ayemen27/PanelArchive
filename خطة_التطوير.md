...
### 🟤 المرحلة الثامنة: توافق Replit الكامل (أولوية حرجة)

#### المهمة 8.1: إصلاح المسارات الثابتة في الوحدات القديمة
- **الحالة**: ⏳ لم تبدأ
- **المسؤول**: في انتظار التعيين
- **الأولوية**: حرجة ⭐⭐⭐
- **المدة المتوقعة**: 2-3 ساعات
- **الوصف**:
  - إصلاح `os.chdir("/www/server/panel")` في class/nginx.py
  - إصلاح `os.chdir("/www/server/panel")` في class/apache.py
  - جعل المسارات ديناميكية تعتمد على البيئة
  - استخدام `public.get_panel_path()` للحصول على المسار الصحيح
- **التسليمات**:
  - [ ] تحديث class/nginx.py
  - [ ] تحديث class/apache.py
  - [ ] تحديث class/ols.py (إن لزم الأمر)
  - [ ] تحديث class/config.py (إن لزم الأمر)
  - [ ] اختبار الاستيراد في Replit
- **معايير القبول**:
  - [ ] `from BTPanel import app` ينجح في Replit
  - [ ] لا توجد أخطاء FileNotFoundError
  - [ ] التطبيق يعمل في كلا البيئتين
  - [ ] اختبارات LSP تمر بنجاح

**الحل المقترح:**
```python
# في class/nginx.py و class/apache.py
# بدلاً من:
...
try:
    ...
    if os.path.exists(panel_path):
        ...
        import sys
        panel_path = os.path.dirname(os.path.abspath(sys.argv[0]))
        if panel_path:
            os.chdir(panel_path)
except Exception as e:
    # في حالة الفشل، استمر دون تغيير المسار
    pass
```

#### المهمة 8.2: إنشاء ملف .env للتطوير
- **الحالة**: ⏳ لم تبدأ
- **المسؤول**: في انتظار التعيين
- **الأولوية**: متوسطة ⭐⭐
...
- **الوصف**:
  ...
  - التأكد من تعيين PORT=5000
- **التسليمات**:
  - [ ] ملف .env جاهز
  - [ ] جميع المتغيرات المطلوبة موجودة
- **معايير القبول**:
  - [ ] التطبيق يقرأ الإعدادات من .env
  - [ ] لا توجد تحذيرات missing variables
...
- **الوصف**:
  - تحديث BTPanel/__init__.py لاستخدام مسارات نسبية
  - إنشاء مجلد logs في المسار الحالي
  - معالجة الأخطاء عند فشل الكتابة
- **التسليمات**:
  - [ ] تحديث setup_logging() في BTPanel/__init__.py
  - [ ] إنشاء logs/ directory
  - [ ] معالجة أخطاء read-only filesystem
- **معايير القبول**:
  - [ ] لا توجد تحذيرات عن /app/logs
  - [ ] الـ logs تُكتب في ./logs/ بدلاً من /app/logs/
  - [ ] التطبيق يعمل حتى لو فشلت الكتابة
...
try:
    ...
    # في حالة الفشل، استخدم /tmp
    log_dir = "/tmp/aapanel_logs"
    try:
        os.makedirs(log_dir, exist_ok=True)
    except:
        # إذا فشل كل شيء، لا تستخدم file logging
        log_dir = None
```

#### المهمة 8.4: اختبار التشغيل الكامل في Replit
- **الحالة**: ⏳ لم تبدأ
- **المسؤول**: في انتظار التعيين
- **الأولوية**: حرجة ⭐⭐⭐
- **المدة المتوقعة**: 1 ساعة
...
- **معايير القبول**:
  - [ ] `python runserver.py` ينجح دون أخطاء
  - [ ] التطبيق يستمع على المنفذ الصحيح
  - [ ] الواجهة يمكن الوصول إليها
...
- **الوصف**:
  ...
  - إضافة قسم Replit Setup
- **التسليمات**:
  - [ ] replit.md محدث
  - [ ] DEVELOPER_GUIDE.md محدث
  - [ ] توثيق خطوات التشغيل في Replit
- **معايير القبول**:
  - [ ] التوثيق يعكس الحالة الفعلية
  - [ ] خطوات واضحة للتشغيل في Replit
...
**المدة الإجمالية المتوقعة للمرحلة**: 5-6 ساعات
