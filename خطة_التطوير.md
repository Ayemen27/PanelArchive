...
- **التسليمات**:
  - [ ] استيراد `DatabaseConnectionPool` في `BTPanel/__init__.py`
  - [ ] إنشاء instance عام من db_pool
  - [ ] تحديث جميع اتصالات قاعدة البيانات لاستخدام db_pool
  - [ ] تحديث health endpoints للتحقق من db_pool
  - [ ] اختبار الاتصالات
  - [ ] تحديث `تقارير_مراجعة_ارشكتر.md` (المشكلة #2)

- **معايير القبول**:
  - [ ] db_pool مستورد ومُستخدم في BTPanel
  - [ ] جميع اتصالات DB تستخدم pooling
  - [ ] health checks تعمل بشكل صحيح
  - [ ] pool statistics متاحة
  - [ ] اختبارات ناجحة
...
- **معايير القبول**:
  - [ ] cron job نشط للنسخ اليومي
  - [ ] اختبار backup ناجح
  - [ ] اختبار restore ناجح
  - [ ] SHA-256 + HMAC verification يعمل
  - [ ] cleanup تلقائي للنسخ القديمة
...
- **معايير القبول**:
  - [ ] SECRET_KEY موجود في .env
  - [ ] لا تحذيرات عند التشغيل
  - [ ] sessions ثابتة بعد restart
...
- **معايير القبول**:
  - [ ] /health/live يرجع `{"status": "ok"}`
  - [ ] /health/ready يرجع حالة DB
  - [ ] /health/metrics يرجع Prometheus metrics
