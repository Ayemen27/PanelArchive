...
if [ "${UBUNTU_VER}" == "18" ] || [ "${UBUNTU_VER}" == "20" ] || [ "${UBUNTU_VER}" == "22" ] || [ "${UBUNTU_VER}" == "24" ];then
	debPath="u${UBUNTU_VER}"
elif [ "${DEBIAN_VER}" == "10" ] || [ "${DEBIAN_VER}" == "11" ] || [ "${DEBIAN_VER}" == "12" ] || [ "${DEBIAN_VER}" == "13" ]; then
...
Install_Freetype()
{
	if [ ! -f "/usr/bin/freetype-config" ] && [ ! -f "/usr/local/freetype/bin/freetype-config" ]; then
		if [ "${DEBIAN_VER}" == "11" ] || [ "${UBUNTU_VER}" == "20" ] || [ "${DEBIAN_VER}" == "12" ] || [ "${UBUNTU_VER}" == "22" ] || [ "${UBUNTU_VER}" == "24" ];then
			cd /usr/local
...
Install_Aptlib(){
	...
	if [ "${DEBIAN_VER}" == "12" ] || [ "${UBUNTU_VER}" == "22" ];then
         apt-get install debian-keyring debian-archive-keyring build-essential gcc g++ make cmake autoconf automake re2c wget cron bzip2 libzip-dev libc6-dev file flex vim bison m4 gawk less cpp binutils diffutils tar libbz2-dev libncurses5 libncurses5-dev libtool libevent-dev openssl libssl-dev libsasl2-dev libltdl3-dev libltdl-dev zlib1g zlib1g-dev libbz2-1.0 libglib2.0-0 libglib2.0-dev libkrb5-dev libpq-dev libpq5 gettext libxml2-dev libcap-dev ca-certificates libc-client2007e-dev psmisc patch git libc-ares-dev libicu-dev e2fsprogs libxslt-dev libc-client-dev xz-utils libgd3 libgd-dev mcrypt libmcrypt-dev libmcrypt4 libmhash-dev libmhash2 libwebp-dev libvpx-dev libfreetype6-dev libjpeg-dev -y --force-yes
	else
